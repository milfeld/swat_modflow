...
#ifdef SHM_IO
#     define read(x,y) k=k+1; READ( dataHRU(startHRU(k):endHRU(k)),y )
#     define iff(x)             if( dataHRU(startHRU(k):endHRU(k)) == shm_eof )
#else
#     define iff(x) if( x )
#endif
...
#ifdef SHM_IO
#     undef  read(x,y  )
#     define read(x,y,z) k=k+1; READ(dataHRU(startHRU(k):endHRU(k)),y,z)
#endif
